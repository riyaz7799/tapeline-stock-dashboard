...
export function createCircuitBreaker<TArgs extends any[], TResult>(
  ...
  const wrapped = async (...args: TArgs): Promise<TResult> => {
    ...
    try {
      ...
      // Success: reset the failure count and close the circuit. This
      // matters even when this particular call didn't start as the
      // recognized HALF_OPEN probe — with several requests firing in a
      // burst (e.g. 5 tickers polling at once), a call that started while
      // CLOSED can still finish successfully after a sibling call already
      // tripped the breaker OPEN. Without this, the breaker could stay
      // OPEN indefinitely despite the API clearly being reachable.
      failureCount = 0;
      if (wasHalfOpen) halfOpenInFlight = false;
