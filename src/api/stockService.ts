...
// Several tickers poll at the same 10s interval, so without this they'd
// all hit Finnhub in the same instant every cycle — a burst pattern that
// free-tier rate limiting treats more harshly than the same total volume
// spread out. This serializes real API calls with a small minimum gap.
const MIN_GAP_MS = 220;
let paceQueue: Promise<void> = Promise.resolve();

function pace(): Promise<void> {
  const next = paceQueue.then(() => new Promise<void>((resolve) => setTimeout(resolve, MIN_GAP_MS)));
  paceQueue = next;
  return next;
}
